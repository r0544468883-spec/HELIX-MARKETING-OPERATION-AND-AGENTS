...
| **DV360** | ✅ | ✅ | ✅ | ⚠️ | ➖ | **new 2026-08-27** — DSP for premium/publisher inventory via Deal ID (PMP/PG/Preferred). Requires a DV360 seat (Google-certified partner); acts at line-item level. insights = Bid Manager report (needs `report_query_id`) |
...
- **DV360** — `advertiser_id`, `access_token` (+ optional `partner_id`, `report_query_id`). Acts at line-item level (`ref.adId` = lineItemId).
  Env: `DV360_ADVERTISER_ID`, `DV360_ACCESS_TOKEN`, `DV360_PARTNER_ID`, `DV360_REPORT_QUERY_ID`.
  **⚠️ Requires a DV360 seat first** — not open self-serve; provisioned via a Google-certified partner. The
  channels UI shows an account-opening notice on the DV360 card. `fetchInsights` reads the latest FINISHED
  **Bid Manager** report for `report_query_id` (create the query once in DV360), downloads the CSV and sums it
  (optionally filtered to the line item) — returns null until a report is ready. `createCampaign` returns
  `dv360_requires_deal` (buys activate a negotiated Deal ID via the Managed pipeline, not a self-serve spec);
  `uploadCreative` unsupported (creatives served via the deal). Mirrors the official `@googleapis/displayvideo` +
  Bid Manager v2 API shapes, kept dep-free like the other connectors — unverified against a live seat.
