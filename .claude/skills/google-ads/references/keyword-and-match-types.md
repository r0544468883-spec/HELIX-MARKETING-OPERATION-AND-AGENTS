...
2. **Mine search terms weekly.** Add winners as keywords, add junk as negatives.
...
- Tag each keyword by intent tier (brand, high, competitor, problem-aware) so reporting maps to the intent ladder.
...
Avoid the same keyword in multiple ad groups or campaigns at the same match type. Use negatives to route each query to exactly one place, or you split data and bid against your own account.
