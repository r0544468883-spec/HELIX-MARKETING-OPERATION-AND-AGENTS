// Custom templates — upload your OWN templates for any message type.
//   GET    ?workspace=&kind=            → list custom templates
//   POST   { workspace_id?/workspace?, kind, key, definition }   → upsert one
//   POST   { workspace_id?/workspace?, templates: [{kind,key,definition}, ...] } → bulk
//   DELETE ?workspace=&kind=&key=       → remove one
// kind ∈ 'whatsapp'. WhatsApp customs still need /api/templates/sync + Meta
...
function ws(request: NextRequest, body?: { workspace_id?: string; workspace?: string }): string | null {
  return body?.workspace_id || body?.workspace || request.nextUrl.searchParams.get('workspace') || process.env.DEFAULT_WORKSPACE_ID || null;
}
