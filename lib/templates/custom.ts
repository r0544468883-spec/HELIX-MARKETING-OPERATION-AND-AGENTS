// Custom (workspace-uploaded) templates — a workspace defines its OWN templates for
// any message type, merged over the built-in catalog (a custom key OVERRIDES a
// built-in one). Backed by the custom_templates table (migration-v17).
...
export async function upsertCustom(workspaceId: string, kind: CustomKind, key: string, definition: unknown): Promise<{ ok: boolean; error?: string }> {
  ...
  const db = createAdminClient();
  if (!db) return { ok: false, error: 'no admin client (missing service key)' };
...
/** Minimal shape validation so a bad upload can't break the notifier/senders. */
